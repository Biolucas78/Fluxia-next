...
console.log('Projeto encontrado em:', projectRoot);

// ─── 1. Adicionar campos em lib/types.ts ───
const typesPath = path.join(projectRoot, 'lib/types.ts');
let typesContent = fs.readFileSync(typesPath, 'utf8');

if (!typesContent.includes('paymentMethod')) {
  const OLD_TYPES = `  productDescription?: string;`;
  const NEW_TYPES = `  paymentMethod?: 'boleto' | 'pix' | 'transferencia' | 'dinheiro' | 'cartao';
  paymentStatus?: 'pendente' | 'pago' | 'vencido';
  paymentDate?: string;
  paymentDueDate?: string;
  productDescription?: string;`;
  typesContent = typesContent.replace(OLD_TYPES, NEW_TYPES);
  fs.writeFileSync(typesPath, typesContent, 'utf8');
  console.log('OK: Campos de pagamento adicionados em types.ts');
} else {
  console.log('SKIP: Campos de pagamento já existem em types.ts');
}

// ─── 2. Criar app/financeiro/page.tsx ───
fs.mkdirSync(path.join(projectRoot, 'app/financeiro'), { recursive: true });
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <Header title="Financeiro" subtitle="Controle de recebimentos" />
...
fs.writeFileSync(path.join(projectRoot, 'app/financeiro/page.tsx'), pageContent, 'utf8');
console.log('OK: app/financeiro/page.tsx criado.');

// ─── 3. Adicionar Financeiro na Sidebar ───
const sidebarPath = path.join(projectRoot, 'components/Sidebar.tsx');
let sidebarContent = fs.readFileSync(sidebarPath, 'utf8');

if (!sidebarContent.includes('financeiro')) {
  // Adicionar import do ícone
  const OLD_ICON = `import { LayoutDashboard, Plus, Factory, Truck, Settings, User, Package, Kanban, RefreshCcw, Loader2, Archive, Trash2, X, Sun, Moon } from 'lucide-react';`;
  const NEW_ICON = `import { LayoutDashboard, Plus, Factory, Truck, Settings, User, Package, Kanban, RefreshCcw, Loader2, Archive, Trash2, X, Sun, Moon, DollarSign } from 'lucide-react';`;
  sidebarContent = sidebarContent.replace(OLD_ICON, NEW_ICON);

  // Adicionar item no nav
  const OLD_NAV = `    { id: 'recorrencia', title: 'Recorrência', icon: RefreshCcw, href: '/recorrencia', roles: ['admin', 'gestor_vendas'] },`;
  const NEW_NAV = `    { id: 'recorrencia', title: 'Recorrência', icon: RefreshCcw, href: '/recorrencia', roles: ['admin', 'gestor_vendas'] },
    { id: 'financeiro', title: 'Financeiro', icon: DollarSign, href: '/financeiro', roles: ['admin'] },`;
  sidebarContent = sidebarContent.replace(OLD_NAV, NEW_NAV);

  fs.writeFileSync(sidebarPath, sidebarContent, 'utf8');
  console.log('OK: Financeiro adicionado na Sidebar.');
} else {
  console.log('SKIP: Financeiro já está na Sidebar.');
}

console.log('OK: Tudo criado com sucesso.');